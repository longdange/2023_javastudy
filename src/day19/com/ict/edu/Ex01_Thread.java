...
//멀티스레드:하나의 프로세스에서 여러 개의 스레드가 병행적으로 처리되는 것
//싱글스레드:하나의 프로세스에 하나의 스레드가 일처리하는 것(day01~day18)
//자바에서 스레드 생성법
//**자바에서는 start()를 호출하면 run()가 실행 되면 스레드 실행 됐다고 함
//1.Thread클래스 상속 받기:start(),run()매서드가 모두 있다.
...
public static void main(String[] args) {
	//지금은 싱글스레드이다.즉, 하나의 스레드가 모든 일을 다 처리한다.
	//Thread.currentThread().getName(): 현재 일하는 스레드 이름 출력
	//main매서드에서 일하는 스래드를 main스레드라고 한다.
